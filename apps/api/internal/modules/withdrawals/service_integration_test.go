...
var (
	withdrawalHarnessOnce sync.Once
	withdrawalHarness     *withdrawalIntegrationHarness
	withdrawalHarnessErr  error
)

type withdrawalIntegrationHarness struct {
...
func TestMain(m *testing.M) {
	...
	if withdrawalHarness != nil {
		withdrawalHarness.close()
	}

	os.Exit(code)
}

func TestBootstrapUsesDefaultWithdrawalFeeWhenIncomeRowMissingIntegration(t *testing.T) {
	harness := requireWithdrawalHarness(t)
	harness.reset(t)

	service := NewService(harness.pool, nil, nil)
	...
	if result.FeePercentage != defaultWithdrawalFeePercentage {
		t.Fatalf("expected default withdrawal fee %d, got %d", defaultWithdrawalFeePercentage, result.FeePercentage)
	}
...
func requireWithdrawalHarness(t *testing.T) *withdrawalIntegrationHarness {
	t.Helper()

	withdrawalHarnessOnce.Do(func() {
		withdrawalHarness, withdrawalHarnessErr = startWithdrawalHarness()
	})

	if withdrawalHarnessErr != nil {
		t.Skipf("skip withdrawal integration harness: %v", withdrawalHarnessErr)
	}

	return withdrawalHarness
}

func startWithdrawalHarness() (*withdrawalIntegrationHarness, error) {
	port, err := freePort()
	if err != nil {
		return nil, err
	}

	basePath, err := os.MkdirTemp("", "justqiuv2-withdrawals-pg-*")
	if err != nil {
	...
	config := embeddedpostgres.DefaultConfig().
		...
		Port(uint32(port)).
		Database("justqiuv2_withdrawals_test").
		Username("postgres").
		Password("postgres").
	...
	if err := applyWithdrawalSchema(context.Background(), pool); err != nil {
		pool.Close()
		_ = db.Stop()
		return nil, err
	}

	return &withdrawalIntegrationHarness{db: db, pool: pool}, nil
}

func applyWithdrawalSchema(ctx context.Context, pool *pgxpool.Pool) error {
...
func (h *withdrawalIntegrationHarness) reset(t *testing.T) {
	t.Helper()
...
func (h *withdrawalIntegrationHarness) close() {
	if h == nil {
		return
	}
	if h.pool != nil {
